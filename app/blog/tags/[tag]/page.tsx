...
import { getTags } from "@/app/utils/tags";
...
export default function TagPage({ params }: { params: { tag: string } }) {
  ...
  const pages = blogs.getPages().filter((blog) =>
    blog.data.tags.some(
  ...
  return (
    <main className="my-16 flex w-full flex-1 flex-col gap-5">
      <div className="mb-5 flex flex-col gap-5">
        <h1 className="mb-4 text-center text-3xl font-bold">{`帶有「${decodedTag}」書籤的文章`}</h1>
      ...
      <div className="grid grid-cols-1 gap-5 md:grid-cols-2 lg:grid-cols-3">
        {pages.map((page) => (
          <BlogItem key={page.file.name} page={page} />
        ))}
      </div>
    </main>
  );
